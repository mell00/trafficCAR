c#' Simplify a built network object by removing parallel edges (and loops)
#'
...
#'
#' @return A network list with updated `edges`, `graph`, and `A` (and same `nodes`).
#' @export
simplify_network <- function(net, keep_edge = c("first", "shortest")) {
  ...
  if (!all(req %in% names(net))) stop("`net` must be a network list from build_network().")

  edges <- net$edges
  nodes <- net$nodes

  if (!inherits(edges, "sf")) stop("`net$edges` must be an sf object.")
  if (!all(c("from", "to") %in% names(edges))) stop("`net$edges` must have `from` and `to` columns.")

  if (nrow(edges) == 0L) {
    g <- igraph::make_empty_graph(n = nrow(nodes), directed = FALSE)
    A <- Matrix::Matrix(0, nrow(nodes), nrow(nodes), sparse = TRUE)
    return(within(net, { graph <- g; A <- A; edges <- edges }))
  }

  # remove any loops defensively
  edges <- edges[!is.na(edges$from) & !is.na(edges$to) & (edges$from != edges$to), ]
  ...
  if (keep_edge == "first") {
    ...
    # shortest by st_length; ties keep first
  ...
  # rebuild graph from simplified edges, keeping isolates
  g <- igraph::make_empty_graph(n = nrow(nodes), directed = FALSE)
  ...
  # adjacency (0/1, symmetric, no diagonal)
